...
if __name__ == '__main__':
    ...
    # ... (al final del script, después de write_track)
    import pandas as pd

    # Crear un DataFrame con los resultados
    # Nota: ball_track tiene el formato [(x1, y1), (x2, y2), ...]
    df = pd.DataFrame(ball_track, columns=['x', 'y'])
    df.insert(0, 'frame', range(len(df))) # Añadir número de fotograma

    # Guardar a CSV
    df.to_csv('trayectoria_pelota.csv', index=False)
    print("Coordenadas guardadas en trayectoria_pelota.csv")
